...
void can_interface_init(void)
{
    ...
    36MHz / 6 -> 6MHz
    6MHz / (1tq + 6tq + 5tq) = 500kHz => 500kbit
    */
    can_init(CAN, // Interface
             ...
             CAN_BTR_TS2_5TQ, // Time segment 2 time quanta width
             6, // Prescaler
             false, // Loopback
             false); // Silent
...
// clock config for external HSE 8MHz cristal
static const my_clock_scale_t clock_72mhz = {
    .pllpre = RCC_CFGR2_PREDIV_HSE_IN_PLL_DIV_NONE,
...
//    gpio_clear(GPIOA, GPIOA_CAN_SPEED);
    ...
    // LED on
    gpio_mode_setup(GPIOC, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIOC_LED);
    gpio_set_output_options(GPIOC, GPIO_OTYPE_PP, GPIO_OSPEED_2MHZ, GPIOC_LED);
